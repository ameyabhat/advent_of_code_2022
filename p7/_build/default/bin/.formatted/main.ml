...
let input = List.map ~f:String.strip (List.filter (String.split (In_channel.read_all "input.txt") ~on:'$') ~f:(fun a -> not (String.is_empty a)))
let _ = new_directory "/" None

let execute_command (dir: directory) (command: string):  directory = 
  let handle_ls ls_command = 
    match String.split ls_command ~on:' ' with
      | ["dir"; dname] -> let _ = add_directory dname (Some dir) in ()
      | [fsize; fname] -> insert_file fname (Int.of_string fsize) dir
      | _ -> failwith "unexpected input to ls" in
  match Str.string_before command 2 with
  | "cd" -> (match List.nth_exn (String.split command ~on:' ') 1 with
              | ".." -> Option.value_exn dir.parent
              | dname -> find_dir dname dir)
  | "ls" -> let ls_seqs = (String.split ~on:'\n' command) in
           let ls_commands = List.sub ls_seqs ~pos:1 ~len:(List.length ls_seqs - 1) in
...
let () = List.iter ~f:(fun s -> let mid = Str.string_before s 2 in print_endline mid) input 
